#' Locally change-point detection using the mmkh approach
#'
...
lacpd_mmkh <- function(x,m=1,k=2,blow=0.1,bup=(1-blow),leave=FALSE,adjust=FALSE,history=FALSE,...){
    ...
    if(k==2){
      ...
      zs.low <- rowMeans(do.call(cbind,out.z.low),na.rm=TRUE)

      out.mag.low <- lapply(X=1:m, function(i){
        out.low[[i]]$mag
      })
      mag.low <- rowMeans(do.call(cbind,out.mag.low),na.rm=TRUE)
      ...
      zs.up <- rowMeans(do.call(cbind,out.z.up),na.rm=TRUE)

      out.mag.up <- lapply(X=1:m, function(i){
        out.up[[i]]$mag
      })
      mag.up <- rowMeans(do.call(cbind,out.mag.up),na.rm=TRUE)
