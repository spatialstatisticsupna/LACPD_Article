...
load("./part1_AR_BY.RData")
load("./part2_AR_BY.RData")
load("./part3_AR_BY.RData")
load("./false_AR_BY.RData")
...
S[which.max(colMeans(allzs1))]
S[which.max(colMeans(allzs2))]
S[which.max(colMeans(allzs3))]


S[which(colMeans(allps1)<0.05)]
S[which(colMeans(allps2)<0.05)]
S[which(colMeans(allps3)<0.05)]
...
png("ARshift-mk-BY-pz.png",width = 1600, height = 1600)
ggplot(dpz,aes(x=x))+geom_line(aes(y=y),lwd=2)+
  ...
  geom_vline(data = data.frame(xint=33,type="p-value",ID="40"), color="gold", aes(xintercept = xint),lwd=1.5,linetype="dashed")+
  geom_vline(data = data.frame(xint=71,type="p-value",ID="80"), color="gold", aes(xintercept = xint),lwd=1.5,linetype="dashed")+
  geom_vline(data = data.frame(xint=112,type="p-value",ID="120"), color="gold", aes(xintercept = xint),lwd=1.5,linetype="dashed")+
  geom_vline(data = data.frame(xint=47,type="p-value",ID="40"), color="gold", aes(xintercept = xint),lwd=1.5,linetype="dashed")+
  geom_vline(data = data.frame(xint=90,type="p-value",ID="80"), color="gold", aes(xintercept = xint),lwd=1.5,linetype="dashed")+
  geom_vline(data = data.frame(xint=129,type="p-value",ID="120"), color="gold", aes(xintercept = xint),lwd=1.5,linetype="dashed")+
